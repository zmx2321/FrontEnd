...
$(function () {
    ...
    var thisApi = {
        ...
        cancel: {
            ...
            product: "/wx/school/v1.0/ssj/lol/cancel"
        },
        branch: {
            dev: "mock/dynamics.json",
        ...
        $.get(bMock.getFace("recommend"), function (data, status) {
            ...
            $.each(data.data,function(i,v){
               ...
               var pp= imgages;
                ...
                var section1="";
                section1+= '<div class="teac2 teac">'+'<div class="teacher-in">'+'<img src="http://static.iboom.tv/static/img/'+data.data[i].head+'" >'+'<div class="teacherInt2 teacherInt">'+'<h3>'+data.data[i].name+'</h3>'+'<a href="javascript:;">明天预约</a>'+'</div>'+'<div class="introduce">'+'<div class="introduce-one">'+'<p>授课范围:</p>'+'<p>累计完成:</p>'+'<p>擅长英雄:</p>'+'</div>'+'<div class="introduce-two2 introduce-two">'+'<p class="imags">'+data.data[i].teachRange+'</p>'+'<p>' + data.data[i].num + '次生死局助教</p>'+'<p >'+pp+'</p>'+'</div>'+'</div>'+'<div class="clear"></div>'+'<p class="introduce-p">'+'<a class="introduce-th" href="ssjservice.html?'+data.data[i].userId+'">预约老师</a>'+'</p>'+'</div>'+'</div>'
            ...
            if(data.data.length===0) {
               $(".new-dyn").hide()
                ...
                $.each(data.data, function (i, v) {
                    ...
                    var section1 = "";
                    section1 += '<div class="dynamices">' + '<div class="dynamic-fir">' + '<span>'+teacherName+'和'+studentNum+'完成生死局助阵。</span>' + '<a class="firsta" href="javascript:;">' + longtime + '</a>' + '</div>' + '<div class="dynamic-two1 dynamic-two">' + '<img src="http://static.iboom.tv/static/img/'+teacherHead+'" >' + '<div>' + '<h3>'+teacherName+'</h3>' + '<span>' + teahsay + '</span>' + '</div>' + '</div>' + '<div class="dynamic-two2 dynamic-two">' + '<img src="'+stupic+'" >' + '<div>' + '<h3>'+studentNum+'</h3>' + '<span>' + stusay + '</span>' + '</div>' + '</div>'
