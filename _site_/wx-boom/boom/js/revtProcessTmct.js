...
//获取老师信息
  ...
  //传入老师ID，找到老师数据
    ...
    var im='<img src="http://static.iboom.tv/static/img/'+teacherinfo.head+'"">'
    $(".proteh_cott ").append(im);
    ...
    var imgages="";
    $.each(getTeacher(window.location.search.substring(1)).goodAtChampions,function(k,v){
...
//获取学生时间
    function getApplyStudent() {
        $.get(bMock.getFace("canApplyStudent")+'?'+'studentNum='+studentNum+'&date='+getNextYearDay(new Date().getTime(),0), function (data, status) {
            if(data.data!=true){
                ...
                $(".revpros_btn button").css("background","#ccc");
            }else{
                $(".protmc_time_cont2").css("display","none");
                $(".protmc_time_cont").show();
                $(".revpros_btn button").css("background","-webkit-linear-gradient(left, #4295ff, #36b5ff")
            };
        });
    ...
    //英雄专攻选择
  $("#HeroFocus").click(function(){
      var ims="";
          for(var k=1;k<hero.data.length;k++){

          var im1="";

           im1 += '<dd id="'+hero.data[k].id+'">'+'<div class="tmct_hreo_pic">'+'<img src="http://static.iboom.tv/static/img/'+hero.data[k].ename+'.png">'+'</div>'+'<span class="tmct_hreo_name">'+hero.data[k].title+'</span>'+'</dd>'

          ims+=im1;
          }
      $(".revtProcessTmct_cont dl ").append(ims);


      $(".revtProcessTmct_bomx").show();
      $(".tmct_hreo_pic img").click(function(){
          $("#HeroFocus").text($(this).parent().siblings().text())
          $("#HeroFocus").parent().attr("id",$(this).parent().parent().attr("id"))
          $(".revtProcessTmct_bomx").hide()
      })
  })
    $(".repoTmct_btn").click(function(){
        $(".revtProcessTmct_bomx").hide()
    })
...
//得到时间的index
    ...
    $(".revpros_btn button").click(function(){
        ...
        if($("#HeroFocus").text()!="英雄选择" && $(".protmc_time_cont > ul > li > dl > dd > ol > li").hasClass("protmc_time_on") ){
          var yeayy=new Date(new Date().getTime()).getFullYear();
        ...
        function GetJsonData() {
            var json = {
                ...
                "whatStudy":JSON.stringify({
                    ...
                    hero:$("#HeroFocus").text(),
                    heroId:$("#HeroFocus").parent().attr("id")
                            }) ,
