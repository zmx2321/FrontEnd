...
//传入老师ID，找到老师数据
    ...
    var im='<img src="http://static.iboom.tv/static/img/'+getTeacher(window.location.search.substring(1)).head+'"">'
    $(".proteh_cott ").append(im);
    var imgages="";
    $.each(getTeacher(window.location.search.substring(1)).goodAtChampions,function(k,v){

        var images1="";
        images1+='<li>'+'<img class="in-img" src="http://static.iboom.tv/static/img/'+hero.data[v].ename+'.png">'+'</li>';
        imgages+=images1;
    });

    $(".protmc_teh_cnt > dl > dd:last-child > ul > li:last-child ol ").append(imgages);
...
//预约时间模块(上一天，下一天）
    ...
    $(".revpros_btn button").click(function () {
        if($(".protmc_time_cont > ul > li > dl > dd > ol > li").hasClass("protmc_time_on") ){
