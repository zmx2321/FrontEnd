    ...
    function getdata(id, sid) {

        $.ajax({
            url: dataport.classmatesA+sid,
            type: "get",
            timeout: publicFunc.time,
            success: function(data) {
                var Func=function(){
                    var record = []
                    record = data.slice(-30);
                    
                    $("#nearlydays").html(record.length)
                    cml(record, id)    
                }
                watchFunc.iflogin(data,Func);
            },
    ...
    function cml(record, id) {
        ...
        var max = minTomax[minTomax.length - 1] + 20;
