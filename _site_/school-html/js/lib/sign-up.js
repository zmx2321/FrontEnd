...
/* --------------------- 获取基础信息 --------------------  */
  ...
  $scope.applyData={
  	...
  	classId	    : myLoginInfo.classId,
  	qq          : myLoginInfo.qq,
  	tier        : myLoginInfo.tier,
  	queue       : myLoginInfo.queue,
    phone       : myLoginInfo.phone,
...
ajax.get(dataport.teacherCalendar,
  function(data){
    if(!data.error){
      ...
      if(data.data.length>0){
        for(var i=0;i<data.data.length;i++){
          // var v=data.data[i]
            data.data[i].goodChampions=[]
          if(data.data[i].teacherInfo){
            if(data.data[i].teacherInfo.goodAtChampion){
              for(var i1=0;i1<data.data[i].teacherInfo.goodAtChampion.length;i1++){
             
                data.data[i].goodChampions.push(lolObj[data.data[i].teacherInfo.goodAtChampion[i1].championId])
              }
              console.log(data.data[i].goodChampions.length)
              if(data.data[i].goodChampions.length>8){
                data.data[i].goodChampions.length=8
              }
            }else{
          ...
          if(data.data[i].doingApply.length>20){
              data.data[i].doingApply.length=20
          }
      ...
      $scope.tArr=data.data||[]

      // console.log($scope.tArr)
...
function tSlide(data){
      $scope.tShowIndex=0;
      $scope.tSd="n"

      $scope.nowTeacher=data[$scope.tShowIndex]

      $scope.tShowFunc=function(a){
        $scope.tShowIndex+=a
        $scope.tShowIndex=$scope.tShowIndex%data.length;
        if($scope.tShowIndex<0){
          $scope.tShowIndex=data.length-1;
        }
         // console.log($scope.index)
         $scope.nowTeacher=data[$scope.tShowIndex]

        // console.log($scope.tShowIndex)
        if(a==1){
          $scope.tSd="n"
        }else{
          $scope.tSd="p"
    ...
    $scope.$watch('tShowIndex',function(n,o){
       //console.log(n)
        //console.log($scope.tArr[n])
        $scope.applyData.teacherId=$scope.tArr[n].teacherId
    })
...
$scope.calendarUrl=dataport.calendarNow;
    ...
    //   $http.get(dataport.calendarOther+"?date="+a+" 00:00",{contentType: 'application/json;charset=UTF-8',datatype:"json"}).success(function(data,status,headers,config){ 
  ...
  // $http.get($scope.calendarUrl,{contentType: 'application/json;charset=UTF-8',datatype:"json"})
...
/* --------------------- step4  提交 --------------------  */
  ...
  $scope.applyOne=function(){
    ...
    $http.post(dataport.applyOnetoOne,$scope.applyData,{datatype:"json"}).success(function(data){
...
mainApp.filter('heroFilter',function(){
 ...
 /* --------------------- doApply --------------------  
 ==================================================================*/
mainApp.filter('doApply',function(){
