    ...
    if(data.error){}
      ...
      if(data.data){
        ...
        for(var i=0;i<data.data.length;i++){
          console.log(i)
          var v=data.data[i]
          if(teacherId==v.userId){
            console.log(v)
            v.goodChampions=[]
            if(v.goodAtChampion.length>1){
              if(v.goodAtChampion.length>8){
                v.goodAtChampion.length=8
              }

              for(var i1=0;i1<v.goodAtChampion.length;i1++){
                v.goodChampions.push(lolObj[v.goodAtChampion[i1].championId])
              }
            }
            console.log(v)
            $scope.teacher=v
...
$scope.myLoginInfo=myLoginInfo
  ...
  $http.post(dataport.teacherSpecific+teacherId,{page:0,size:10},{datatype:"json"})
  .success(function(data){
      // console.log(data)
     $scope.pagesArr={
      now:data.nowPage,
...
// dataport.applyPraise=publicport+"boot/oneToOne/applyPraise" //评论点赞 studentNum,applyId//点赞
 ...
 $scope.applyPraise=function(a,b,c){
    ...
    $http.get(dataport.applyPraise+"?studentNum="+a+"&applyId="+b,{contentType: 'application/json;charset=UTF-8',datatype:"json"})
    .success(function(data){
