...
var app = new Vue({
    ...
    activeName2: 'first',
		...
		// boardUrl1: 'https://yun.tim.qq.com/v4/ilvb_edu/whiteboard?sdkappid=1400042983&contenttype=json',
		roomList: [],
		createInfo: {
			conf_name: '',
			owner_nick: ''
	...
	watch: {
      ...
      if (!value) {
		...
		// TICSDK.CONSTANT.EVENT.IM.RECEIVE_BOARD_ROOM_MSG: function () {},
		tab (val) {
			if (val === 'camera') {
                document.getElementById('emd2').style.display='none';
				// $('.edu-member-list').css('height', 0)
				// document.getElementById('eMD1').style.height = 0
			/*	document.getElementById('eMD2').style.height = 0
				document.getElementById('emd2').style.height = 0*/
			} else {
        // document.getElementById('eMD1').style.height = '95px'
        /*document.getElementById('eMD2').style.height = '95px'
        document.getElementById('emd2').style.height = '95px'*/
        document.getElementById('emd2').style.display='block';
      ...
      console.log(tab, event);
		...
		login: function (id, pwd) {
			...
			self.sendRequest({
				...
				// self.boardUrl1 += '&usersig=' + res.user_sig + '&identifier=' + id;
				// 这个时候得登陆IM
							...
							elems.forEach(function (elem) {
								...
								if (elem.type == 'TIMTextElem') {
									...
									if (type == 'TXWhiteBoardExt') {
										...
										self. 吧 && self.paint.addData(JSON.parse(elem.content.data));
									}
				...
				onGroupSystemNotifys: {
					255: function (res) {
						...
						if (data.sub_cmd == 'member_join_notify') {
							...
							if (self.role == 'student' && self.conf_info.home_screen != tempScreen && (self.conf_info.home_screen.indexOf('whiteboard') == -1 || self.conf_info.home_screen.indexOf('whiteboard1') == -1)) {
		...
		joinConf: function (id, nick_name) {
          ...
          if (self.role == 'student') {
            ...
            self.tab = ret.home_screen || 'camera';
            if (self.tab.indexOf('whiteboard1') > -1) {
              self.changeConf({
                home_screen: this.tab
              });
              if (!self.isFirstSwitchBoard) {
		...
		switchTab: function (tab) {
			...
			if (this.conf_info.home_screen != tab) {
				...
				if ((tab == 'whiteboard' || tab == 'whiteboard1')) {
					console.log('第一次切到白板，初始化白板');
					self.isFirstSwitchBoard = true;
					Vue.nextTick(function () {
						self.teacherInit();
    ...
    toWhiteBoard: function () {
      ...
      self.ticSdk.init()
      self.ticSdk.on(TICSDK.CONSTANT.EVENT.IM.RECEIVE_BOARD_ROOM_MSG,res => {
      	console.log('qwawedfgrhtt')
      	console.log(res)
        self.paint.addData(res);
      });
		},
		// 老师
		teacherInit: function () {
			var self = this;
      var pad_id = 'sketch_pad';
      self.paint = new Sketch({
        id: pad_id,
        conf_id: self.conf_id,
        user: self.createInfo.owner_nick,
    ...
    getBoardList1: function () {
			// var self = this;
      this.paint.getBoardList()
			console.log(this.paint.getBoardList())
		},
