<template>
    <section class="main_cont">
            ...
            <el-table class="flow_list" :data="flow_info" border highlight-current-row v-loading="listLoading" height="calc(100vh - 218px)">
                ...
                <el-table-column fixed="right" label="操作" width="75">
                    <template slot-scope="scope">
                        <el-button type="text" size="small" @click="editFlow(scope.row)" v-on:click="editFlowVisible = true">编辑</el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog title="编辑统计" :close-on-click-modal="false" :visible.sync="editFlowVisible" :before-close="handleClose">
            <el-form :model="editFlowData" status-icon :rules="editFlowRules" ref="editFlowForm" label-width="160px">
                <el-form-item label="PV值" prop="pv">
                    <el-input v-model="editFlowData.pv" placeholder="请输入PV值" clearable></el-input>
                </el-form-item>
                <el-form-item label="UV值" prop="uv">
                    <el-input v-model="editFlowData.uv" placeholder="请输入UV值" clearable></el-input>
                </el-form-item>
                <el-form-item label="注册数量" prop="registerNum">
                    <el-input v-model="editFlowData.registerNum" placeholder="请输入注册数量" clearable></el-input>
                </el-form-item>
                <el-form-item label="通过数量" prop="passNum">
                    <el-input v-model="editFlowData.passNum" placeholder="请输入通过数量" clearable></el-input>
                </el-form-item>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 点击编辑
            editFlow (row) {
                this.editFlowData = Object.assign({}, row);
                this.editFlowData.flowId = row.id;
            },
            // 提交编辑表单
            editFlowSubmit (formName) {
                // 验证表单
                this.$refs[formName].validate((valid) => {
                    this.listLoading = true;
                    ...
                    //如果验证成功，请求接口数据
                    if (valid) {
                        if (!Number.isInteger(parseInt(this.editFlowData.pv)) && this.editFlowData.pv < 0) {
                            this.$message.warning("pv 必须为大于0的正整数");
                        } else if (!Number.isInteger(parseInt(this.editFlowData.uv)) && this.editFlowData.uv < 0) {
                            this.$message.warning("uv 必须为大于0的正整数");
                        } else {
                            editFlow(qs.stringify(this.editFlowData)).then(res => {
                                // console.log(res);
